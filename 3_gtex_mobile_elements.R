...
dt.out = read.delim('output/mobile.element.tsv', stringsAsFactors = F)
dt.out$fdr.range = as.character(dt.out$fdr.range)
dt.out$col = unlist(sapply(dt.out$fdr.range, switch, 'Not significant'='grey', 'FDR 25%'='grey', 'FDR 20%'='grey', 'FDR 15%'='khaki3',
...
legend('topright', pch=19, pt.cex=1.3, inset=c(-0.45,0), xpd = TRUE, bty = "n", 
...
kgp.sine = genic.plot[genic.plot$feature == 'SINE',]
kgp.sine$cohort = 'KGP'
kgp.line = genic.plot[genic.plot$feature == 'LINE',]
kgp.line$cohort = 'KGP'
...
gtex.sine = genic.plot[genic.plot$feature == 'SINE',]
gtex.sine$cohort = 'GTEX'
gtex.line = genic.plot[genic.plot$feature == 'LINE',]
gtex.line$cohort = 'GTEX'


sine = rbind(kgp.sine, gtex.sine)
sine$move = sapply(sine$cohort, switch, "GTEX" = 0.2, "KGP" = (-0.2))
sine$dist = gsub('KB', '',sine$extension)
sine$dist = as.numeric(as.character(sine$dist))

sine = sine[order(sine[,13], sine[,12]),] 
genic.plot = sine
pdf('output/figures/SINE_gtex_kgp.pdf', w = 6, h = 4)#, h = 7
par(mar = c(9,6,1,8), mgp = c(3,1,0), mfrow=c(1,1)) 
plotCI((1:10)+genic.plot$move, genic.plot$OR, ylab = 'OddsRatio (variable vs stable)', xlab = 'Distance from TE',
       main = '', ui = genic.plot$OR.upper, li = genic.plot$OR.lower, pch = NA,
       lwd = 1, xaxt="n", ylim = c(0,3), col = 'black', cex = 1, yaxt = 'n', )
points((1:10)+genic.plot$move, genic.plot$OR, pch = genic.plot$pch, bg = genic.plot$col, cex = 1.5, col = 'black')
...
line = rbind(kgp.line, gtex.line)
line$move = sapply(line$cohort, switch, "KGP" = 0.2, "GTEX" = (-0.2))

line = rbind(kgp.line, gtex.line)
line$move = sapply(line$cohort, switch, "GTEX" = 0.2, "KGP" = (-0.2))
line$dist = gsub('KB', '',line$extension)
line$dist = as.numeric(as.character(line$dist))
class(line$dist)

line = line[order(line[,13], line[,12]),] # , line$cohort
genic.plot = line
pdf('output/figures/LINE_gtex_kgp.pdf', w = 6, h = 4)#, h = 7
par(mar = c(9,6,1,8), mgp = c(3,1,0), mfrow=c(1,1)) 
plotCI((1:10)+genic.plot$move, genic.plot$OR, ylab = 'OddsRatio (variable vs stable)', xlab = 'Distance from TE',
       main = '', ui = genic.plot$OR.upper, li = genic.plot$OR.lower, pch = NA,
       lwd = 1, xaxt="n", ylim = c(0,3), col = 'black', cex = 1, yaxt = 'n', )
...
text((1:5)*2, par("usr")[3]-0.5, srt = 0, adj = 1, xpd = TRUE, cex = 1, 
     labels = paste(unique(genic.plot$extension)))
legend('topright', pch=17, pt.cex=1.3, inset=c(-0.45,0), xpd = TRUE, bty = "n", 
       col = c('red','orange', 'yellow', 'khaki3','grey'), 
       legend=c( "FDR 1%","FDR 5%","FDR 10%","FDR 15%", "Not significant"), title = 'FDR range')
legend('topright', pch=24, pt.cex=1.3, inset=c(-0.45,0), xpd = TRUE, bty = "n", col = 'black',
       bg = c('red','orange', 'yellow', 'khaki3','grey'), 
       legend=c( "FDR 1%","FDR 5%","FDR 10%","FDR 15%", "Not significant"), title = '')
dev.off()