...
### L1 Combined
line.kgp = read.delim('../_1KGP/output/LINE.L1.sub.families.tsv')
line.kgp$cohort = 'KGP'
line.gtex = read.delim('output/LINE.L1.sub.families.tsv')
line.gtex$cohort = 'GTEX'

line = rbind(line.kgp, line.gtex)
line$move = sapply(line$cohort, switch, "GTEX" = 0.2, "KGP" = (-0.2))
line = line[order(line[,1], line[,9]),] 
line$col = unlist(sapply(line$fdr.range, switch, 'Not significant'='grey', 'FDR 25%'='grey', 'FDR 20%'='grey', 'FDR 15%'='khaki3',
                         'FDR 10%'='yellow','FDR 5%'='orange', 'FDR 1%'='red'))

genic.plot = line
pdf('output/figures/L1_families_gtex_kgp_names_.pdf', w = 6.5, h = 4)
par(mar = c(9,6,1,8), mgp = c(3,1,0), mfrow=c(1,1)) 
plotCI((1:18)+genic.plot$move, genic.plot$OR, ylab = 'OddsRatio (variable vs stable)', xlab = '',
       main = '', ui = genic.plot$OR.upper, li = genic.plot$OR.lower, pch = NA,
       lwd = 1, xaxt="n", ylim = c(0,4), col = 'black', cex = 1, yaxt = 'n', )
points((1:18)+genic.plot$move, genic.plot$OR, pch = 24, bg = genic.plot$col, cex = 1.5, col = 'black')

axis(1, at=seq(1, 18, by=2)+0.5, labels = FALSE)
axis(2, at=seq(0, 6, by=1), labels = TRUE, las = 2)
abline(h = 1, lwd = 0.5, lty = 2)

text((1:9)*2-0.5, par("usr")[3]-0.5, srt = 90, adj = 1, xpd = TRUE, cex = 1, 
